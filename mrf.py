...
def parse(inputfile, outputfile=None):
    ...
    data_col5 = extract_cross_from_list(data_col5, column_name_list)
    column_name_list = "provider_references"
    data_col5 = extract_cross_from_list(data_col5, column_name_list)
    ...
    required_columns = ['reporting_entity_name', 'npi', 'negotiation_arrangement', 'code_type', 'code', 'modifier',
                        'negotiated_type', 'billing_class', 'place_of_service_code', 'rate']
    ...
    df["place_of_service_code"] = df["place_of_service_code"].astype(np.int64)
    ...
    return df  # parse_simple(inputfile, outputfile)
...
def parse_simple(inputfile, outputfile=None):
    ...
    for num, item in enumerate(data):
        name = transform_name(item.get('name'), num)
        type = transform_type(item.get('type'), num)
        breed = transform_breed(item.get('breed'), num)
        try:
            dob = item.get('born')
        except:
            dob =""
        output.append([name, type, breed, dob])

    df = pd.DataFrame(output, columns=['name', 'type', 'breed', 'dob'])
...
if __name__ == '__main__':
    ...
    # python3 mrf.py test/bcbstn.json